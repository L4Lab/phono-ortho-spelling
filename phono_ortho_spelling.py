...
def load_everything_in():
    for i in range(1, len(input_file)):
       ...
       if "true" in line[2].lower():
           line[2]= "1"
       if "false" in line[2].lower():
           line[2] = "0"

       cons = open("cons.csv").readlines()
       if(len(line) == 3):
           for con in cons:
               con = con.split(",")
               if(con[0].lower() == line[0].lower()):
               	   line.append('"'+line[1]+'"')
...
def unpack_nouns():
	...
	print(len(in_hi))
	for noun in in_low:
		low.append(noun.name.lower())
	for noun in in_hi:
		hi.append(noun.name.lower())
	print()
	in_low = low
	print("low",in_low)
	print()
...
# Pick 12 words for test
# UPDATED: to do 10 words now.
def pick_12():
    global dicts
    global in_hi
    global in_low
    ...
    # 5 words that have low variability
    while i < num:
        sel_word = randint(0,len(dicts)-1)
        if(dicts[sel_word] in new_dicts or dicts[sel_word]['Word'] in in_hi):
            continue
        else:
            new_dicts.append(dicts[sel_word]) 
            i += 1
    i = 0
    # 5 words that have high variability
    while i < num:
        sel_word = randint(0,len(dicts)-1)
        if(dicts[sel_word] in new_dicts or dicts[sel_word]['Word'] in in_low):
            continue
        else:
            new_dicts.append(dicts[sel_word]) 
            i += 1
...
def use_pretest_nouns(nouns): 
    ...
    for word in dicts:
        if test or see:
            ...
            print(word)
        for noun in nouns:
           if word["Word"].lower() == noun.name.lower():
    ...
    def load(self, *args):
        ...
        if(to_Open == '()' or to_Open == ""):
            ...
            if(".csv" in to_Open):
            	...
            	in_file = open("output_pretest/"+file_name).readlines()

            	p_test = open('pretest.csv', 'w')
            	
            	first_line = ",ORTHO TARGET,PRODUCTION,T/F,Condition\n"
            	in_file.insert(0,first_line)

            	i = 0
            	p_test.write(in_file[0])
            	for line in range(1,len(in_file)):
            		if "True" in in_file[line]:
            			line.replace("True", "1")
            		if "False" in in_file[line]:
            			line.replace("True", "0")
            		p_test.write(str(i)+","+in_file[line])
            		i += 1
            	p_test.close()
            	self.ready = True
            	self.incorrect_file = False
...
def assign_nouns(short_nouns, long_nouns):
    ...
    print("create new")
    in_low = lo_variability
    in_hi = hi_variability
    print("end create")
        ...
        for post in test[1:]:
            ...
            for pre in pretest[1:]:
                ...
                if post[1].lower() == pre[1].strip('"').lower():
                    ...
                    cor = pre[3].strip('"')
                    num = -1
                    if cor == "FALSE" or cor == "false":
                    	num = 0
                    elif cor == "TRUE" or cor == "true":
                    	num =  1
                    else:
                    	num = -1
                    if num == 1:
